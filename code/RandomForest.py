...
for i in range(20):
    ...
    rowList = np.array(random.sample(range(0, trained_dataset.shape[0]), int(trained_dataset.shape[0] * np.random.rand())))
    dropRows(new_trained_dataset, rowList)
    colList = [trained_dataset.columns[i] for i in np.array(random.sample(range(1, trained_dataset.shape[1]), int(trained_dataset.shape[1] * np.random.rand())))]
